...
byte_depth = 2
print("Byte_Depth: ", byte_depth)

'''
msg = "Heeeey Sid, How ya Doin' ?!"
s = msg.encode('ascii')
print(s)
s_encode = base64.b64encode(s).decode('ascii').replace('=','')
print("~~~~~~~~~~~~~~~~~~~~~> ",s_encode)
encode_ar=np.array(list(s_encode))
print(encode_ar)
def b64_bin(x,d):
    return d[x]
...
print("Msg bits:  ",len(s_bin))
px_ht = math.ceil((len(s_bin)/3))
px_wd = 1
if px_ht>1024:
    px_wd = math.ceil(len(s_bin)/1024/3/byte_depth)
    px_ht = 1024
print("No of PixelsRows: ",px_ht)
print("No of PixelsColumns: ",px_wd)
#print(s_bin)


img = Image.open("org.png").convert('RGB')
img_ar = np.array(img)

select_px = img_ar[:px_wd+1, :px_ht+1]
print(select_px.shape)
plt.imshow(select_px)
plt.show()
print(select_px.shape)

print("Select Pixel Count: ",len(select_px))
flat_px = select_px.flatten()
print(flat_px)
zip_bin = np.array(list(zip(flat_px,ch_bin)), dtype=object)
print("Select Pixel Bits: ", len(flat_px))
