    ...
    for z in Grids_ar:
        ...
        if z not in CheckList:
            ...
            if Grids[z][0] is not None:
                ...
                encode_bin = []
                for y in encode_ar:
                    encode_bin.append(b64[y])
                encode_bin = np.array(encode_bin)
                print(encode_bin)
                ...
                for x in zip_bin:
                    ...
                    encoded_px.append(int(bin_str,2))
                encoded_px = np.array(encoded_px)
...
steg(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9])
t2_ = time.time()
