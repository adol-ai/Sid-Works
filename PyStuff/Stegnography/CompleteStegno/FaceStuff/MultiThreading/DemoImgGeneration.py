...
def steg(OutFile, SeedValue, BioTone, VoiceFile, Face1, Face2, Face3, Face4, Face5, Palm=None, Iris=None, Ear=None):
    ...
    def process(z):
        ...
        if z not in CheckList:
            ...
            if z == "BioTone":
                ...
                ar = np.random.randint(255, size=org_dim).astype(np.uint8)
                img = Image.fromarray(ar)
            
            img_ar = np.array(img)
            dim = img_ar.shape
            
            print(Fore.MAGENTA+Style.BRIGHT+"instance_data.byte_depth: ", byte_depth, Fore.RESET)
            
            if Grids[z][0] is not None:
                with open(Grids[z][0], 'rb') as f:
                    b64_str = f.read()
                    ...
                    if byte_depth <= 0 or byte_depth > 8:
                        raise ValueError("Invalid Byte Depth Value")
                    if len(s_bin) > (math.prod([dim[0], dim[1], 3, byte_depth])):
                        ...
                        raise ValueError("Insufficient Memory Spacing, Increase Byte Depth Value")
    ...
    if __name__ == '__main__':
        with concurrent.futures.ProcessPoolExecutor() as executor:
            executor.map(formation, Grids_ar)
        
if __name__ == '__main__':
    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.submit(steg, *sys.argv[1:10])
